...
- Runtime-bin companion crate (deferred item `runtime-bin-delivery`): platform matrix follows the upstream published wheel matrix (which includes Windows x64 and macOS x64; macOS needs the sibling `-spawn-helper`) — see the README's `Runtime acquisition` → Route B for the current wheel list.
