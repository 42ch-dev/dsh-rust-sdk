...
severity: high
plan_id: 003-upstream-launch-contract
tags:
  - upstream-drift
  - launch-contract
  - dsh
...
The re-alignment was executed as iteration `003-upstream-launch-contract` and frozen in two repo-level specs that are now the contract SSOT:
...
- **Documenting the old world in the README**: the README pair claimed the interactive `dsh` CLI was "not this SDK's runtime" and advertised a 3-platform matrix, while the CLI *is* the runtime carrier and upstream publishes 5 targets. Replaced by a docs-truth pass restating every fact from the frozen specs (compass AC7; plan 07).
